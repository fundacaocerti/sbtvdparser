...
public class CC extends PES {
	...
	public static boolean onlyCC = false;

	public static int singleProgramId = 0;

	public static void reset() {
	}

	public CC(final int pid, final int pgNumber, final int cTag) {
		id = 0xbd;
		this.pid = pid;
		name = Messages.getString("CC.caption") + " prog:" + pgNumber + " tag:" + BitWise.toHex(cTag) + " pid:" + pid; //$NON-NLS-1$
	...
	public void printHeader() {
		if (bigBuffer.length > 46) super.printHeader();
		else
			if (mgmtPackets == 0) mgmtPackets = addSubItem("Management packets"); //$NON-NLS-1$
			else {
				int i = bw.getAbsolutePosition();
				bw.setOffset(bigBuffer.length - 2);
				final int crc = bw.pop16();
				bw.setOffset(i);
				for (i = 0; i < mgmtCount; i++)
					if (mgmtCrcs[i] == crc) return;
	...
	public void parse() {
		...
		/*
		 * data_identifier 0x80 private_stream_id 0xFF PES_data_packet_header_length Shows the length of
		 * PES_data_private_data_byte. Normally 0x00 is input. *2
		 */
		final int[] prefix = { 0x80, 0xff, 0xf0 };
		for (int i = 0; i < prefix.length; i++)
			if (prefix[i] != bw.pop()) return;
		addSubItem(Messages.getString("CC.startOk"), thisPacket); //$NON-NLS-1$
		while (bw.getAvailableSize() > 7) {// 5b de header + 2 de crc = mínimo
			// dataGroup
			final int crcStart = bw.getAbsolutePosition();
			String id = "A-"; //$NON-NLS-1$
			final int data_group_id = bw.consumeBits(6);
			if (data_group_id > 0x1F) id = "B-"; //$NON-NLS-1$
			if (data_group_id % 0x20 == 0) id += "management"; //$NON-NLS-1$
			else id += "statement lang. " + data_group_id % 0x20; //$NON-NLS-1$
			...
			final int data_group_size = bw.pop16();
			addSubItem("data_group_size: " + data_group_size, dataGroupLvl); //$NON-NLS-1$

			// bw.printBuffer(bw.getAbsolutePosition(), bw.getBufferSize());
			if (data_group_id % 0x20 == 0) parseCCManagement(bw, data_group_size);
			else parseCCStatement(bw, data_group_size);

			final int readCrc = bw.pop16();
			final int crc = CRC16.calc(bigBuffer, crcStart, data_group_size + 5); // 5
			// bytes no header
			addSubItem("CRC16: " + BitWise.toHex(crc) + " " + (readCrc == crc), dataGroupLvl); //$NON-NLS-1$ //$NON-NLS-2$
		}
	}

	void parseTMD(final BitWise bw, final boolean parseFree) {
		final String[] tmdTxt = { "Free", "Real-time", "Offset time", "reserved" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
		// TMD 2
		bw.remainingBits = 0;
		final int tmd = bw.consumeBits(2);
	...
	void parseDataUnit(final BitWise bw) {
		// bw.printBuffer(bw.getAbsolutePosition(), bw.getAbsolutePosition()+4);
		final int data_unit_loop_length = (bw.pop16() << 8) + bw.pop();
		final int duLvl = addSubItem("data_unit", dataGroupLvl); //$NON-NLS-1$
		addSubItem("data_unit_loop_length: " + data_unit_loop_length, duLvl); //$NON-NLS-1$
		if (data_unit_loop_length < 5) return;
		addSubItem("unit separator: " + (0x1f == bw.pop()), duLvl); //$NON-NLS-1$
		final int[] typeId = { 0x20, 0x28, 0x2c, 0x30, 0x31, 0x34, 0x35 };
		final String[] typeNames = { "text", "geometric", "sound", "1b DRCS", "2b DRCS", "color map", "bitmap" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$ //$NON-NLS-7$
		String typeName = Messages.getString("CC.unknown"); //$NON-NLS-1$
		final int type = bw.pop();
		for (int i = 0; i < typeNames.length; i++)
			if (type == typeId[i]) typeName = typeNames[i];
		addSubItem(Messages.getString("CC.duType") + typeName, duLvl); //$NON-NLS-1$
		final int data_unit_size = (bw.pop16() << 8) + bw.pop();
		addSubItem(Messages.getString("CC.duSize") + data_unit_size, duLvl); //$NON-NLS-1$
		final int contentLvl = addSubItem(Messages.getString("CC.duContent"), duLvl); //$NON-NLS-1$
		if (type == typeId[0]) new CCPresentation(contentLvl, thisPacket).parse(bw, data_unit_size);
	}

	void parseCCStatement(final BitWise bw, final int size) {
		final int mark = bw.getAbsolutePosition();
		parseTMD(bw, true);
		while (bw.getAbsolutePosition() - mark < size)
			parseDataUnit(bw);
	}

	void parseCCManagement(final BitWise bw, final int size) {
		bw.mark();
		// bw.printBuffer(bw.getAbsolutePosition(),
		// bw.getAbsolutePosition()+10);
		parseTMD(bw, false);
		// num_languages 8
		final int num_languages = bw.pop();
		...
		for (int i = 0; i < num_languages; i++) {
			...
			bw.consumeBits(1);
			// DMF 4 bslbf
			final int dmf = bw.consumeBits(4);
			final String[] dispType = { "Auto", "Non", "Select", "Specific" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
			addSubItem("dmf: " + dispType[BitWise.stripBits(dmf, 4, 2)] + " display", dataGroupLvl); //$NON-NLS-1$ //$NON-NLS-2$
			if (dmf == 12 || dmf == 13 || dmf == 14) addSubItem("dc: " + bw.pop()); //$NON-NLS-1$
			...
			final int df = bw.consumeBits(4);
			final String[] hv = { "Horizontal", "Vertical" }; //$NON-NLS-1$ //$NON-NLS-2$
			final String[] dens = { "standard dens.", "high density", "western lang.", "1920x1080", "960x540", "720x480", //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
