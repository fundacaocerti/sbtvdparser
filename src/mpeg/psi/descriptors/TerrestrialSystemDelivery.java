...
public class TerrestrialSystemDelivery extends Descriptor {
	...
	public void printDescription() {
		...
		addSubItem("guard_interval: 1/" + (32 >> (BitWise.stripBits(tmp, 4, 2))), level);

		// tx_mode 2 uimsbf
		addSubItem("transmission_mode: mode " + (1 + (BitWise.stripBits(tmp, 2, 2))), level);
		...
		for (int i = 2; i < descriptor_length; i += 2) {
			...
			addSubItem("frequency: " + freq + ".143 MHz - channel " + chan, level);
